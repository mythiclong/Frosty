...
public class Xray extends Module {
    ...
    private List<ColoredBox> targets = List.of();
    private ScanTask scanTask;
    private boolean lastEspEnabled;
    private MultiBufferSource.BufferSource boxBuffer;
    ...
    public void onRender3D(Render3DEvent event) {
        ...
        if (boxBuffer == null) {
            boxBuffer = RenderUtils.getVCP();
        }

        // Draw each ore box
        PoseStack matrix = event.getMatrix();
        float alpha = opacityAlpha() / 255.0f;
        float lineWidth = 2.0f;
        for (ColoredBox target : targets) {
            if (esp.isToggled()) {
                RenderUtils.drawBoxFilled(matrix, target.box, target.color, true);
            }
            if (outline.isToggled()) {
                RenderUtils.drawBox(matrix, target.box, target.color, lineWidth, true);
            }
        }
    }
    ...
    private final class ScanTask {
        ...
        private final ArrayList<ColoredBox> found = new ArrayList<>();
        ...
        private List<ColoredBox> results() {
    ...
    public static class ColoredBox {
        ...
        public ColoredBox(AABB box, Color color) {
            this.box = box;
            this.color = color;
        }
    }
