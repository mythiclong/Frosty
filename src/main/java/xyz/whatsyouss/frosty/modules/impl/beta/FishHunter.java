...
public class FishHunter extends Module {

    // Settings
    private final SliderSetting range = new SliderSetting("范围", 1, 32, 16, 1);
    private final SliderSetting minDistance = new SliderSetting("最小距离", 1, 10, 3, 0.5);
    ...
    private Entity findTarget(Minecraft mc) {
        ...
        List<Entity> entities = mc.level.getEntities(mc.player, searchBox, this::isFishEntity);
    ...
    private boolean isFishEntity(Entity entity) {
    ...
    private void selectBestWeapon() {
        ...
        for (int i = 0; i < 9; i++) {
            ...
            // Simple damage check (could be improved)
    ...
    public String getDesc() {
        return "自动狩猎附近的鱼类（鳕鱼、鲑鱼、热带鱼）。状态: " + state.name();
    }
}
